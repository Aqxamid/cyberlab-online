  ...
  <div class="rounded-xl p-4 flex gap-3" style="background:#271c00;border:1px solid #9e6a03">
    ...
    <div>
      ...
      <p class="text-xs mt-1" style="color:#b08800">
        ...
        <code class="px-1 rounded" style="background:#3d2e00">fetch('/api/flag').then(r=>r.json()).then(d=>alert(d.flag))</code>
      </p>
    </div>
  </div>
...
<script>
  const flagPayload = '<img src=x onerror="fetch(\\'/api/flag\\').then(r=>r.json()).then(d=>{document.getElementById(\\'flag-display\\').textContent=\\'🏁 \\'+d.flag;document.getElementById(\\'flag-display\\').classList.remove(\\'hidden\\')})">';

  document.getElementById('hint-alert').addEventListener('click', () => { document.getElementById('search-q').value='<img src=x onerror=alert(1)>'; doSearch(); });
  document.getElementById('hint-flag').addEventListener('click',  () => { document.getElementById('search-q').value=flagPayload; doSearch(); });
  document.getElementById('cmnt-hint-flag').addEventListener('click', () => { document.getElementById('c-text').value=flagPayload; });
  ...
  async function loadComments(){
    try{const list=await fetch('/api/vulnerable/comments').then(r=>r.json());
    document.getElementById('comments-list').innerHTML=list.map(c=>'<div class="p-3 rounded-lg" style="border:1px solid #30363d"><div class="flex items-center gap-2 mb-1"><span class="text-xs font-semibold" style="color:#58a6ff">'+c.author+'</span><span class="text-xs" style="color:#484f58">'+new Date(c.timestamp).toLocaleString()+'</span></div><div class="text-sm" style="color:#c9d1d9">'+c.text+'</div></div>').join('');}catch(e){console.error('Failed to load comments',e);}
  }

  async function doPatchSearch(){
    const q=document.getElementById('p-search').value;
    try{const d=await fetch('/api/patched/search?q='+encodeURIComponent(q)).then(r=>r.json());document.getElementById('p-search-out').textContent='Sanitized: '+d.reflected;}catch(e){document.getElementById('p-search-out').textContent='Error: '+e.message;}
  }
