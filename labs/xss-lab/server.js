...
app.get('/api/flag', (req, res) => {
  res.json({ flag: 'FLAG{xss_reflected_pwned}' });
});
...
<div class="max-w-4xl mx-auto px-4 py-8 space-y-6">
  ...
  <div class="rounded-xl p-4 flex gap-3" style="background:#271c00;border:1px solid #9e6a03">
    ...
    <div>
      ...
      <p class="text-xs mt-1" style="color:#b08800">
        <strong>To get the flag:</strong> inject a script that calls <code class="px-1 rounded" style="background:#3d2e00">fetch('/api/flag')</code> and displays the result.
        Use the hint buttons below if you get stuck.
      </p>
    </div>
  </div>
  ...
  <div class="card p-6 shadow-sm">
    ...
    <div class="pt-4 space-y-2" style="border-top:1px solid #30363d">
      ...
      <input id="c-author" type="text" placeholder="Your name"
        class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none">
      <textarea id="c-text" rows="3" placeholder="Try an XSS payload as your comment..."
        class="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none font-mono resize-none"></textarea>
  ...
  <div class="card p-6 shadow-sm">
    ...
    <div class="flex gap-2 mb-3">
      <input id="p-search" type="text" placeholder="Try same payloads here..."
        class="flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none font-mono">
...
<script>
  ...
  document.getElementById('hint-alert').addEventListener('click', () => {
    document.getElementById('search-q').value = PAYLOADS.alertImg; doSearch();
  });
  document.getElementById('hint-cookie').addEventListener('click', () => {
    document.getElementById('search-q').value = PAYLOADS.cookieImg; doSearch();
  });
  document.getElementById('hint-flag').addEventListener('click', () => {
    document.getElementById('search-q').value = PAYLOADS.flagScript; doSearch();
  });

  document.getElementById('cmnt-hint-alert').addEventListener('click', () => {
    document.getElementById('c-text').value = PAYLOADS.commentAlert;
  });
  document.getElementById('cmnt-hint-flag').addEventListener('click', () => {
    document.getElementById('c-text').value = PAYLOADS.commentFlag;
  });
  document.getElementById('cmnt-normal').addEventListener('click', () => {
    document.getElementById('c-text').value = 'This is a normal comment, nothing suspicious here!';
  });
  ...
  async function loadComments() {
    try {
      const r = await fetch('/api/vulnerable/comments');
      ...
      el.innerHTML = list.map(c =>
        ...
        + '</div>'
        + '<div class="text-sm" style="color:#c9d1d9">' + c.text + '</div>'
        + '</div>'
      ).join('');
  ...
  async function doPatchSearch() {
    ...
    try {
      ...
      document.getElementById('p-search-out').textContent =
        'Sanitized output: ' + d.reflected + '\\n(HTML chars escaped — safe to render)';
    } catch(e) {
      document.getElementById('p-search-out').textContent = 'Error: ' + e.message;
    }
  }
